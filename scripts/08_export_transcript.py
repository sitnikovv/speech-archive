...
def main() -> int:
    ...
    artifacts.ensure_dirs(ROOT); stage=artifacts.stage_dir(ROOT/'data','08','transcript'); stem=f'{base}.transcript.with-names'
    ...
    io_utils.write_text(out,export.render_transcript(merged,speakers)); io_utils.write_json(man,{'stage':'08_transcript','script':'scripts/08_export_transcript.py','input_artifacts':[{'path':str(merged_p),'sha256':io_utils.sha256_file(merged_p)},{'path':str(speakers_p),'sha256':io_utils.sha256_file(speakers_p)}],'output_artifacts':[{'path':str(out),'sha256':io_utils.sha256_file(out)}],'created_at':io_utils.utc_now()})
    print(out); return 0
