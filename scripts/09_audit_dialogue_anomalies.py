...
def main() -> int:
    ...
    artifacts.ensure_dirs(ROOT); stage=artifacts.stage_dir(ROOT/'data','09','audit'); stem=f'{base}.audit.dialogue'
    ...
    data={'artifact':str(out),'stage':'09_audit','source_mp3':merged.get('source_mp3'),'merged_file':str(merged_p),'speakers_file':str(speakers_p),'transcript_file':args.transcript,'created_at':io_utils.utc_now(),**audit.audit_candidates(merged,speakers)}
    ...
    io_utils.write_json(man,{'stage':'09_audit','script':'scripts/09_audit_dialogue_anomalies.py','input_artifacts':inputs,'output_artifacts':[{'path':str(out),'sha256':io_utils.sha256_file(out)}],'created_at':io_utils.utc_now()})
    print(out); return 0
